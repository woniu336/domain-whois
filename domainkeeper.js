...
async function fetchWhoisInfo(domain) {
  try {
    // 使用正确的 URL 格式
    const url = `${WHOIS_PROXY_URL}/${domain}`;
    console.log(`请求的 WHOIS URL: ${url}`); // 打印请求的 URL
    const response = await fetch(url);
    
    // 检查响应状态
    if (!response.ok) {
      const errorText = await response.text(); // 获取错误响应文本
      console.error(`WHOIS API responded with status: ${response.status}, response: ${errorText}`);
      throw new Error(`WHOIS API responded with status: ${response.status}`);
    }
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      console.error('非 JSON 响应:', text); // 打印非 JSON 响应
      throw new Error('返回的响应不是有效的 JSON');
    }
    ...
    return {
      ...
      registrationDate: formatDate(whoisData["Creation Date"]) || 'Unknown',
      expirationDate: formatDate(whoisData["Registry Expiry Date"]) || 'Unknown'
    };
  } catch (error) {
